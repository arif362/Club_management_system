...
if(isset($_POST['form1'])) {
	...
	try {
		...
		$statement = $db->prepare("SELECT * FROM tbl_login WHERE id=2");
		$statement->execute();
		...
		$statement = $db->prepare("UPDATE tbl_login SET username=?, fullname=?, password=?  WHERE id=2");
		$statement->execute(array($_POST['username'],$_POST['fullname'],$new_final_password,));
		
		$success_message = "User Profile has been changed successfully.";
...
<form action="" method="post">
	...
	<?php
	$statement = $db->prepare("SELECT * FROM tbl_login WHERE id=2 ORDER BY id ASC");
	$statement->execute();
	$result = $statement->fetchAll(PDO::FETCH_ASSOC);
	foreach($result as $row)
	{
