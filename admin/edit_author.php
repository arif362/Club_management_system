...
if(isset($_POST['update_author'])) {
	...
	try {
		...
		$statement = $db->prepare("SELECT * FROM author_list WHERE author_id=?");
		...
		$statement = $db->prepare("UPDATE author_list SET  firstname=?, lastname=?, designation=?, sex=?, birthday=?, contact_number=?, account_number=?, address=?, password=?  WHERE author_id=?");
		$statement->execute(array($_POST['firstname'],$_POST['lastname'],$_POST['designation'],$_POST['sex'],$birthday,$_POST['contact_num'],$_POST['account_num'],$_POST['address'],$new_final_password,$author_id));
		
		$success_message = "Author Profile has been changed successfully.";
...
<?php include('header.php');?>
	...
	<h2>Edit Author</h2>
		...
		<?php
					...
					$statement = $db->prepare("SELECT * FROM author_list WHERE author_id=? ORDER By author_id");
					$statement->execute(array($author_id));
					$result = $statement->fetchAll(PDO::FETCH_ASSOC);
	...
	<form action="" method="post">
		...
		<td>
			...
			<option value="General Manager">President</option>
			<option value="Assistant General Manager">Vice-President</option>
			<option value="Manager">Manager</option>
			<option value="Senior Officer">Advisor</option>
			<option value="Junior Officer">Junior Advisor</option>
			<option value="Officer">Cashier</option>
			<option value="Staff">Publisher</option>
			</select>
		</td>
		</tr>
		<tr>
		...
		<tr>
			<td><input type="submit" value="SAVE" name="update_author"></td>
		</tr>
		</table>
