...
if(isset($_POST['add_author'])) {
	   ...
	   // Database duplicate username check setup for use below in the error handling if else conditionals
		 $statement = $db->prepare("SELECT email FROM author_list WHERE username=?");
		 $statement->execute(array($username));
		 $uname_check = $statement->rowCount();
		 
		 // Database duplicate e-mail check setup for use below in the error handling if else conditionals
		 $statement = $db->prepare("SELECT email FROM author_list WHERE email=?");
	...
	try {
		...
		$statement = $db->prepare("INSERT INTO author_list (username,firstname,lastname,type,designation,sex,birthday,contact_number,account_number,address,email,password,add_date) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)");
		$statement->execute(array($username,$_POST['firstname'],$_POST['lastname'],$type,$_POST['designation'],$_POST['sex'],$birthday,$_POST['contact_num'],$_POST['account_num'],$_POST['address'],$email,$password,$add_date));
		
		$success_message ='Your Registration is Complete Successfully.';
...
<?php include('header.php');?>
	...
	<form action="" method="post">
		...
		<tr>
			<td><input class="short" type="text" name="firstname" placeholder="First Name"></td>
		</tr>
		<tr>
			<td><input class="short" type="text" name="lastname" placeholder="Last Name"></td>
		</tr>
		<tr>
		<td>
		...
		<tr>
			<td><input  class="short" type="text" name="contact_num" placeholder="Contact Number"></td>
		</tr>
		<tr >
			<td><input class="short" type="text" name="account_num" placeholder="Account Number"></td>
		</tr>
		...
		<tr>
			<td><input  class="short" type="text" name="email1" placeholder="Email"></td>
		</tr>
		<tr>
			<td><input  class="short" type="text" name="email2" placeholder="Confirm Email"></td>
		</tr>
		<tr>
		...
		<tr>
			<td><input type="submit" value="SAVE" name="add_author"></td>
		</tr>
		</table>	
	</form>
