...
if(isset($_POST['signup'])) {
	   ...
	   // Database duplicate username check setup for use below in the error handling if else conditionals
		 $statement = $db->prepare("SELECT email FROM member_list WHERE username=?");
		 $statement->execute(array($username));
		 $uname_check = $statement->rowCount();
		 
		 // Database duplicate e-mail check setup for use below in the error handling if else conditionals
		 $statement = $db->prepare("SELECT email FROM member_list WHERE email=?");
	...
	try {
		...
		if(empty ($_POST['dept_id'])) {
			throw new Exception('Student id No. can not be empty');
		}
		...
		$statement = $db->prepare("INSERT INTO member_list (firstname,lastname,username,user_type,dept_id,dept_name,sex,contact_num,address,birthday,email,password,regi_date) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)");
		$statement->execute(array($_POST['firstname'],$_POST['lastname'],$username,$_POST['user_type'],$_POST['dept_id'],$_POST['dept_name'],$_POST['sex'],$_POST['contact_num'],$_POST['address'],$birthday,$email,$password,$regi_date));
		
		$success_message ='Your Registration is Complete Successfully .You can Login Now !.';
			...
			<div class="main_registraion">
				...
				<form action="" method="post">
					<table>
						...
						<tr class="contact">
							<td>Student id No   .        :     <input type="text" name="dept_id"></td>
						</tr>
						<tr class="post_type">
							<td>Department : 
